...
export function WalletGatePanel({
  ...
  return (
    <section className="mt-4">
      <Section eyebrow="Wallet" title="Connect to start">
...
export function HeroBootstrapPanels({
  ...
  return (
    <section className="grid gap-5 lg:grid-cols-[1.2fr_0.8fr]">
      <Section eyebrow="Expedition" title={`Locked hero: ${PLAYER_HERO_NAME}`}>
        ...
        <div className="mt-4 grid gap-3">
          {squadSpotlights.map((spotlight) => (
            ...
            >
              ...
              <p className="mt-2 text-sm text-slate-400">{spotlight.vibe}</p>
            </div>
          ))}
        </div>
      </Section>
      ...
      <Section
        eyebrow="Deployment"
        title={`Bootstrap ${PLAYER_HERO_NAME}`}
        actions={
          <button
            type="button"
          ...
          >
            {apiBusy ? "Loading Vault..." : "Start Expedition"}
          </button>
        }
      >
